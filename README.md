...
5. **Пакетная проверка CSV:** используйте панель «Пакетная классификация CSV» или вызовите API напрямую: `curl -X POST -F "file=@data/sample_batch.csv" http://127.0.0.1:8000/predict_file`.
6. **Docker-режим:** `docker compose up --build` поднимет сервис с тем же API и фронтендом (см. раздел 11.7).
