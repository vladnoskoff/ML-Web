...
### 11.2 Быстрый старт
1. **Установите зависимости:** `pip install -r requirements.txt` (FastAPI, scikit-learn, pandas и т.д.).
2. **Обучите модель:** `python ml/train_baseline.py`. Скрипт сохранит пайплайн в `models/baseline.joblib` и метаданные в `models/metadata.json`.
3. **Запустите backend:** `uvicorn backend.app.main:app --reload`.
...
> ⚠️ Если PyPI недоступен, установите зависимости из локального зеркала. Без обученной модели API работает на keyword-фолбэке, поэтому можно тестировать весь стек даже до обучения.
