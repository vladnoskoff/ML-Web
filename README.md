...
3. **Оцените модель:** `make evaluate` прогонит сохранённую модель (формат не важен: sklearn-пайплайн или трансформер) по размеченному датасету и положит отчёт в `reports/eval_metrics.json`.
...
8. **Журнал предсказаний:** когда API соберёт несколько запросов, запустите `make history-report`, чтобы получить агрегированную статистику по файлу `data/prediction_history.jsonl` (результаты сохранятся в `reports/history_summary.json`).
...
- `backend/app/config.py` централизует настройки FastAPI и использует `pydantic.BaseSettings`. Любой параметр можно переопределить через `.env` или переменные окружения с префиксом `APP_` (например, `APP_MODEL_PATH`, `APP_TRANSFORMER_DIR`, `APP_HISTORY_PATH`, `APP_ALLOW_ORIGINS`).
