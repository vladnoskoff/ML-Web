...
3. **Оцените модель:** `make evaluate` прогонит сохранённый пайплайн по размеченному датасету и положит отчёт в `reports/eval_metrics.json`.
...
- `ml/evaluate.py` прогоняет обученную модель по любому размеченному CSV и сохраняет Accuracy, Macro F1, подробный `classification_report` и confusion matrix в `reports/eval_metrics.json` (вызывается через `make evaluate`).
...
- Скрипт `ml/feedback_to_dataset.py` собирает JSONL в CSV c колонками `text`/`label`, чтобы можно было дообучить модель: `make feedback-export`.
